...
impl FileManager {
    ...
    pub fn read(&mut self, block: &BlockId, page: &mut Page) -> Result<()> {
        ...
        file_guard.read_exact(&mut page.contents().as_bytes().to_vec())?;
    ...
    pub fn write(&mut self, block: &BlockId, page: &mut Page) -> Result<()> {
        ...
        file_guard.write_all(page.contents().as_bytes())?;
        file_guard.flush()?;
    ...
    pub fn append(&mut self, filename: &str) -> Result<BlockId> {
        ...
        let block = BlockId::new(filename.to_string(), new_block_number as u64);
    ...
    pub fn length(&mut self, filename: &str) -> Result<usize> {
        ...
        let len = file_guard.metadata()?.len();
...
mod test {
    ...
    fn test_read_write_int() {
        ...
        let filename = "simple_int.db";
        let block = BlockId::new(filename.to_string(), 0);
